...
class Recommender:
    def __init__(self, n_clusters, clustering_features):
        ...
        self.pipeline = Pipeline([("scaler", StandardScaler()), ("kmeans", KMeans(n_clusters=n_clusters, n_init=10))])            
    ...
    def _get_mean_from_tracks(self, track_ids):
        ...
        track_feature_vectors = [self._get_track_feature_vector(track_id) for track_id in track_ids]
    ...
    def _get_track_feature_vector(self, track_id):
        ...
        if track_feature_vector is None:
            raise Exception(f"Track with ID '{track_id}' was not found in CSV or Spotify API")
